...
/**
 * Tests for the BBAStarParallel class
 */
public class BBAStarParallelTest {
    ...
    private Map<String, GraphNode> getBBAStarSolution(String[] file) throws InputFileException {
        ...
        BBAStarParent algorithm = new BBAStarParent(graph, io.getNumberOfProcessorsForTask(), io.getNumberOfProcessorsForParallelAlgorithm());
    ...
    public void testE3Proc2() {
        try {
            assertTrue(solutionIsOptimal(getBBAStarSolution(FILE3_PROC2), 55));
        } catch (InputFileException e) {
            assert(false);
        }
    }
    ...
    public void testE5Proc2() {
        try {
            assertTrue(solutionIsOptimal(getBBAStarSolution(FILE5_PROC2), 350));
        } catch (InputFileException e) {
            assert(false);
        }
    }
